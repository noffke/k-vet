...
- [ ] T080 i18n completeness pass: every user-facing string in `k-vet-web/src/i18n/{de,en}.json` (grep for hardcoded strings), de-DE default verified, `config.example.toml` comments bilingual, and every screen displays/accepts numbers, dates, and money in the active locale's conventions (both locales spot-checked; no raw `toString`/`toFixed` on user-facing values)
...
- [ ] T083 [P] Repo `README.md` (en-US): project overview (what/why, single-binary + PostgreSQL architecture summary), dev setup and commands, contract-regeneration workflow (adapted from quickstart.md), links to `docs/`
- [ ] T084 [P] `docs/installation.md` (en-US, hardware-agnostic — generic Linux): obtain/build the binary (x86_64 or aarch64), PostgreSQL setup (database + user), `config.toml` walkthrough referencing the bilingual `config.example.toml`, attachments directory, systemd unit example, first login; generic backup note (back up the database dump and the attachments directory together). No Pi/SSD specifics, no borgmatic/Prometheus/Healthchecks (operator-personal environment, not public docs)
- [ ] T085 [P] `docs/templates.md` (en-US): template reference — Typst invoice template (data passed in, config override path, example) and minijinja invoice email template (syntax, first-line-subject convention, variable list, umlaut/encoding note per R17/R18)
